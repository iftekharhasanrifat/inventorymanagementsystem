...
const ShowCashInfos = () => {
  const [cashInfos, setCashInfos] = useState([]);
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("https://inventory-management-ba-git-4e9e06-iftekharhasanrifats-projects.vercel.app/cashinfo")
      .then((res) => {
        setCashInfos(res.data.data);
  ...
  useEffect(() => {
    const result = cashInfos.filter((cashInfo) => 
      cashInfo.date.toLowerCase().includes(search.toLowerCase()) 
    );
  ...
  const columns = [
    ...
    {
      name: "Cash Sale Amount",
      selector: (row) => row.cashSaleAmount,
    },
    {
      name: "Expense",
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-row justify-center items-center gap-2 overflow-x-auto">
          <Link to={`/cashinfos/details/${row._id}`}>
            <BsInfoCircle className="text-green-800 text-lg" />
          </Link>
          <Link to={`/cashinfos/edit/${row._id}`}>
            <AiOutlineEdit className="text-yellow-800 text-lg" />
          </Link>
          <Link to={`/cashinfos/delete/${row._id}`}>
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl">Cash Infos</h1>
        <Link to="/cashinfos/create">
